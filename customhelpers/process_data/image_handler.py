...
def crop_innersqr(img_open):
	...
	return img_open.crop((padding_h, padding_v, size_img[0] - padding_h, size_img[1] - padding_v))

def reshape_image(img_open, shape):
	return img_open.resize(shape) 

def serialize_image(img_open):
	img_serialized = np.asarray(img_open)
	return img_serialized.reshape(1, img_serialized.size)[0]
